...
st.sidebar.title("Map Controls ⚙️")
        ...
        try:
            ...
            if value_column:
                st.session_state.uploaded_files[file_key] = {
                    ...
                    "icon": '📌' # Default icon for string data
                }
            else:
...
with st.sidebar.expander("Custom Data Layers", expanded=True):
    if not st.session_state.uploaded_files:
        st.markdown("<p style='font-size: smaller; font-style: italic;'>Upload a CSV file to see custom layers here.</p>", unsafe_allow_html=True)
    else:
                ...
                with col2:
                    if is_numeric:
                        ...
                        file_info['icon'] = st.selectbox(
                            "Icon", 
                            ['📌', '⭐', '🏔️', '🏛️', '🏞️', '🌳', '🏨'], 
                            index=['📌', '⭐', '🏔️', '🏛️', '🏞️', '🌳', '🏨'].index(file_info.get('icon', '📍')),
...
def main():
    ...
    try:
        ...
        st.plotly_chart(fig, use_container_width=True, key="nepal_map")

        # Add the footer below the map
        st.markdown("""
        <div style="text-align: center; padding-top: 20px; font-size: small; color: grey;">
            Made by Prabesh Guragain and Ankit Bhattarai as a project of Data Mining.
        </div>
        """, unsafe_allow_html=True)
        
        event_data = st.session_state.get("nepal_map")
